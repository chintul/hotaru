...
// Slugs only, for generateStaticParams. Deliberately not PRODUCT_DETAIL: the
// build would otherwise pull every image, variant and translation in the shop
// just to learn the paths.
export const PRODUCT_SLUGS = gql`
  query ProductSlugs {
    productCollection(first: 200, filter: { status: { eq: active } }) {
      edges { node { slug } }
    }
  }
`
...
// Your own review for one product, approved or not. The read policy keeps an
// unapproved row visible to its author precisely so the form can say "awaiting
// approval" and offer an edit, rather than looking like the submission vanished.
export const MY_PRODUCT_REVIEW = gql`
  query MyProductReview($productId: UUID!, $profileId: UUID!) {
    reviewCollection(
      first: 1
      filter: { productId: { eq: $productId }, profileId: { eq: $profileId } }
    ) {
      edges { node { id rating title body isApproved isVerifiedPurchase } }
    }
  }
`
...
// `filter` is built by the caller and applied by Postgres, not by the client.
// The admin list used to load a page of rows and filter that page in memory, so
// searching for an order older than the window answered "no such order" — the
// same shape as a real miss. totalCount reflects the filter, which is what lets
// the page say how many matched rather than how many it happens to hold.
export const ADMIN_ALL_ORDERS = gql`
  query AdminAllOrders($first: Int = 50, $filter: OrderFilter) {
    orderCollection(first: $first, filter: $filter, orderBy: [{ placedAt: DescNullsLast }]) {
      totalCount
