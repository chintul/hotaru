    ...
    case 'order_placed_owner':
      return {
        ...
        html: shell(`Шинэ захиалга ${p.order_number}`, `
          <p style="font-size:14px">${p.customer_email} · ${p.customer_phone}</p>
          ${totals(p)}`),
      }
    ...
    case 'payment_submitted_owner':
      return {
        ...
        html: shell('Дансаа шалгана уу', `
          <p style="font-size:14px">${p.customer_email} захиалга ${p.order_number}-ийн төлбөрийг
    ...
    case 'order_oversold_owner':
      return {
        ...
        html: shell('Нөөц хүрэлцэхгүй', `
          ...
          <p style="font-size:14px">${p.customer_email} · ${p.customer_phone}</p>${totals(p)}`),
      }
